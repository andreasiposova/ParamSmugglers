...
def save_bb_sweep_models(entity, project, sweep_id, dataset, subset, type):
    ...
    for run in sweep.runs:
        ...
        model_file.download(root=run_dir, replace=False)

        # Download the config file
        config_file = run.file('config.yaml')
        config_file.download(root=run_dir, replace=False)

sweep_id = '4foffpnp'
save_sweep_models(entity, project, sweep_id, 'adult', 'black_box', 'malicious')
