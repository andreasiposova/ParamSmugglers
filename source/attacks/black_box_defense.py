...
def prune_model(model, activations, layer_size, input_size, dropout, num_hidden_layers, pruning_amount):
...
def eval_defense(config, X_train, y_train, X_test, y_test, X_triggers, y_triggers, column_names, n_rows_to_hide, data_to_steal, hidden_num_cols, hidden_cat_cols, mal_ratio, repetition, mal_data_generation):
    ...
    y_trigger_ints, y_trigger_preds_ints, trigger_acc, trigger_prec, trigger_recall, trigger_f1, trigger_roc_auc, trigger_cm = eval_on_test_set(
        attacked_model, trigger_dataset)
    ...
    for i in pruning_range:
        ...
        if i == 0:
            pruning_amount = 0
            ben_model, benign_pruned_indices = prune_model(benign_model, benign_model_activations, layer_size,
                                                           input_size, dropout, num_hidden_layers, pruning_amount)
            att_model, attacked_pruned_indices = prune_model(attacked_model, benign_model_activations, layer_size,
                                                             input_size, dropout, num_hidden_layers, pruning_amount)
        else:
            pruning_amount = pruned_neurons
            ben_model, benign_pruned_indices = prune_model(ben_model, benign_model_activations, layer_size, input_size, dropout, num_hidden_layers, pruning_amount)
            att_model, attacked_pruned_indices = prune_model(att_model, attacked_model_activations, layer_size, input_size, dropout, num_hidden_layers, pruning_amount)

        layer = benign_model.fcs[0]
