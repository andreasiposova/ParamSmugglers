...
from source.helpers.black_box_helpers import generate_malicious_data, reconstruct_from_preds, baseline
...
from source.evaluation.evaluation import eval_on_test_set, cm_class_acc
...
class WeightModifier:
    ...
    def modify_weights_with_noise(self, percentage_to_modify):
        modified_indices = {}
            ...
            if 'weight' in name:
                with torch.no_grad():
                    ...
                    if available_indices.size(0) < n_modify:
                        break  # Not enough weights left to modify
                    ...
                    # Determine scale for noise (based on smallest weights)
                    ...
                    noise = torch.randn_like(flat_weights[actual_indices_to_modify]) * (max_val_in_range.item() * self.noise_scale)
                    flat_weights[actual_indices_to_modify] += noise  # Add noise
...
def eval_defense(config, X_train, y_train, X_test, y_test, column_names, data_to_steal, hidden_num_cols, hidden_cat_cols):
    ...
    base_modifier = WeightModifier(benign_model, noise_scale=noise_scale)
    ...
    for step in modification_range:
        ...
        ben_model, modified_base_indices = base_modifier.modify_weights_with_noise(percent_to_modify)
        att_model, modified_attack_indices = mal_modifier.modify_weights_with_noise(percent_to_modify)
